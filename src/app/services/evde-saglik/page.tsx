...
export default function EvdeSaglikPage() {
  ...
  return (
    <div className="w-full max-w-full overflow-hidden space-y-3.5 pb-32 animate-in fade-in duration-300">
      ...
      {/* 2. Hero Info Banner */}
      <div className="bg-gradient-to-r from-[#FAF7F2] to-[#F5EFE6] border border-[#E8DFD3] rounded-2xl p-3 flex items-center gap-3 shadow-sm w-full max-w-full overflow-hidden">
        <div className="w-10 h-10 rounded-xl bg-[#C87D55] text-white flex items-center justify-center shrink-0 shadow-md">
          <ShieldCheck className="w-5 h-5" />
        </div>
        <div className="min-w-0 flex-1">
          <h2 className="text-xs sm:text-sm font-extrabold text-[#2D241E] break-words">
            Stressiz & Steril Kapıda Bakım
          </h2>
          <p className="text-xs text-slate-600 leading-tight mt-0.5 break-words">
      ...
      <div className="flex items-center gap-1.5 overflow-x-auto no-scrollbar py-0.5">
        {[
          ...
          <button
            ...
            className={`px-3 py-1.5 rounded-xl text-[11px] sm:text-xs font-bold whitespace-nowrap transition-all flex-shrink-0 ${
              activeFilter === f.id
        ...
        {filteredServices.map((service) => {
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-3.5 border transition-all cursor-pointer relative overflow-hidden w-full max-w-full ${
                isSelected
            ...
            >
              <div className="flex items-start gap-3">
                ...
                {/* Service image */}
                {service.image && (
                  <div className="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 shrink-0 border border-slate-200/60 shadow-xs">
                    <img
                      src={service.image}
                      alt={service.name}
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-start justify-between gap-1">
                    <h3 className="font-heading font-semibold text-base md:text-lg text-[#2D241E] leading-snug break-words">
                      {service.name}
                    </h3>
                    <div className="text-right shrink-0 ml-1.5">
                      <span className="font-extrabold text-xs sm:text-sm text-[#C87D55] block">
                        {service.price} ₺
                      </span>
                      {service.unit && (
                  ...
                  <p className="text-xs md:text-sm text-slate-600 line-clamp-2 leading-relaxed mt-1 break-words">
                    {service.description}
                  </p>
                  ...
                  {service.weightOptions && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {service.weightOptions.map((w, idx) => (
                        <span
                          key={idx}
                          className="bg-[#F5EFE6] text-[#5C3D2E] text-[10px] font-bold px-1.5 py-0.5 rounded-md"
                        >
                          {w}
                        </span>
                      ))}
                  ...
                  {service.requiresNotice && (
                    <div className="mt-1.5 bg-[#FFFBEB] border border-[#FDE68A] text-[#92400E] text-[10px] font-medium p-1.5 rounded-lg flex items-start gap-1">
                      <AlertCircle className="w-3.5 h-3.5 text-[#D97706] shrink-0 mt-0.5" />
                      <span className="leading-tight break-words">{service.noticeText}</span>
                    </div>
                  )}
                  ...
                  {service.features && (
                    <div className="mt-1.5 flex flex-wrap gap-1">
                      {service.features.slice(0, 2).map((feat, idx) => (
                        <span
                          key={idx}
                          className="text-[10px] bg-[#F4EFE6] text-[#6B5A4E] font-medium px-1.5 py-0.5 rounded"
                        >
                          ✓ {feat}
                        </span>
                      ))}
      ...
      {/* 5. Sticky Bottom Action Bar (Tamamen Bottom Nav'ın Üstünde - bottom-20 / bottom-[72px] z-40) */}
        ...
        <div className="bg-[#2D241E] text-white rounded-2xl p-3 shadow-[0_10px_25px_rgba(0,0,0,0.4)] border border-white/10 flex items-center justify-between gap-3 pointer-events-auto backdrop-blur-md">
          <div className="min-w-0">
            <div className="flex items-center gap-1.5">
              <span className="w-5 h-5 rounded-full bg-[#C87D55] text-white text-[10px] font-extrabold flex items-center justify-center shrink-0">
                {selectedIds.length}
              </span>
              <span className="text-xs font-bold truncate">Hizmet Seçildi</span>
            </div>
            <div className="text-sm font-extrabold text-[#F5EFE6] mt-0.5 truncate">
          ...
          <Button
            ...
            className={`px-4 sm:px-5 py-2.5 rounded-xl font-extrabold text-xs sm:text-sm shadow-lg transition-all shrink-0 ${
